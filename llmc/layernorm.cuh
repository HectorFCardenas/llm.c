...
__global__ void layernorm_forward_kernel3(floatX* __restrict__ out, float* __restrict__ rstd,
...
__global__ void layernorm_forward_kernel6(floatX* __restrict__ out, float* __restrict__ rstd,
                                    const floatX*  __restrict__ inp, const floatX*  __restrict__ weight,
                                    const floatX* __restrict__ bias, int N, int C) {
...
//}
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        for(int k = 0; k < x128::size; ++k) {
            ...
            //if (idx == 0 && c == 0) {  // Log for the first element only
    //printf("Normalized output: %f, Final output after weight and bias: %f\n", n, o);
//}
            out_data[k] = (floatX)o;
        }
    ...
    for (int bt = baseIdx; bt < B * T; bt += warpsInGrid) {
        ...
        for (int i = warpThreadIdx * x128::size; i < C; i += WARP_SIZE * x128::size) {
            ...
            for (int k = 0; k < x128::size; k++) {
                ...
                dnorm += dnorm_i * (float)inp128_i[k];
            }
        }

        const float rstd_bt = rstd[bt];
        dnorm = warpReduceSum(dnorm) * rstd_bt / C;
        ...
        for (int c = 0; c < iterations_C; c++) {
            ...
            for(int o = 0; o < x128::size / f128::size; ++o) {
                ...
                for(int i = 0; i < f128::size; ++i) {
                    ...
                    float dval = (float)weight128[x] * (float)dout128[x]; // Gradient calculation
                    dval -= norm_bti * dnorm;  // Adjust using RMS-only term
                    dval *= rstd_bt;  // Scale
...
void layernorm_forward(floatX* out, float* rstd,
    ...
    if (status == cudaSuccess) {
        layernorm_forward_kernel6<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(out, rstd, inp, weight, bias, N, C);
    } else {
        // fall back to the version without shared memory
        const int grid_size_fb = CEIL_DIV(N * WARP_SIZE, block_size);
        layernorm_forward_kernel3<<<grid_size_fb, block_size, 0, stream>>>(out, rstd, inp, weight, bias, N, C);
    }
    cudaCheck(cudaGetLastError());
}
...
void fused_residual_forward5(floatX* residual, floatX* normed, float* rstd,
    ...
    if(status == cudaSuccess) {
        ...
        layernorm_forward(normed, rstd, residual, weight, bias, N, 1, C, stream);
    }
    cudaCheck(cudaGetLastError());
}
...
void layernorm_backward(floatX* dinp, floatX* dweight, floatX* dbias, float* scratch,
    ...
    layernorm_backward_kernel10<<<grid_size, block_size, shared_mem_size, stream>>>(dinp, dweight, dbias, scratch, dout, inp, weight, rstd, B, T, C);
    cudaCheck(cudaGetLastError());
}
